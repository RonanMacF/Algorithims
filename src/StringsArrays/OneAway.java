...
public class OneAway {
	...
	public boolean mergeAll(String first, String second) {
		...
		if (Math.abs(first.length() - second.length()) > 1)
			return false; // If more than 1 char between them then break;
		...
		while (index1 < s1.length() && index2 < s2.length()) {
			...
			// If there is a difference of character and difference seen before return false
			// as 2 changes, if same length increase index1&2, else just 2
